 ...
 *
 * ТЕКУЩИЙ СТАТУС (важно): Тинькофф выдал ключи с пометкой DEMO — это
 * тестовый терминал. Он ДОЛЖЕН СНАЧАЛА получить один тестовый платёж
 * (Тинькофф сам это требует, прежде чем откроет боевой терминал с
 * настоящими деньгами). Ключи ниже — DEMO, реальные деньги ими принять
 * НЕЛЬЗЯ, только тестовые карты. Как только тестовый платёж пройдёт,
 * Тинькофф выдаст боевые TerminalKey/Password — их нужно будет просто
 * вписать сюда вместо текущих, остальной код не меняется.
 *
...
// ТЕСТОВЫЕ (DEMO) КЛЮЧИ — после успешного тестового платежа Тинькофф выдаст
// боевые значения, тогда просто замените их здесь на настоящие:
const TERMINAL_KEY = '1785336284633DEMO';
const TERMINAL_PASSWORD = '6hNMZ9u$2VCKvz#Q';
